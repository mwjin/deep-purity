...
def make_benchmark_plot():
    ...
    """
    # params
    ratios = [0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1]  # tumor ratio for each normal contam
    model_ver = '190523.5-2'  # for deep-purity
    top_lodt_num = 3100 if CELL == 'HCC1143' else 3500  # for heuristic
    ...
    # for absolute
    abs_summary_path = f'/extdata4/baeklab/minwoo/projects/absolute/results/absolute/{CELL}/30x/purity_summary.txt'
    ...
    # for chat
    # chat_summary_path = f'/extdata4/baeklab/minwoo/projects/chat/results/cna-masked-2/{CELL}/30x/purity_summary.txt'
    chat_summary_path = f'/extdata4/baeklab/minwoo/projects/chat/results/my-own-chat/{CELL}_30x_purity_estimation_seg.txt'
    ...
    # for deep-purity
    deep_purity_path = f'/extdata4/baeklab/minwoo/projects/deep-purity/prediction/pred_{model_ver}.{CELL}_summary.txt'
    ...
    heuristic_path = f'/extdata4/baeklab/minwoo/projects/deep-purity/results/heuristic/est-tumor-purity/' \
                     f'top_lodt_purity_{CELL}_30x_{top_lodt_num}.txt'
    ...
    png_outfile = f'{out_dir}/190612_{model_ver}_{CELL}_{top_lodt_num}_junhak-chat-seg.png'
    os.makedirs(out_dir, exist_ok=True)

    plt.figure(figsize=(9, 7))
    plt.title(f'Purity Estimation Benchmark {CELL}', size=15)
    ...
    plt.plot(sorted(ratios), [purity_to_predictions[x][1] for x in sorted(ratios)],
             color='#2020f7', marker='o', linestyle='dashed', markersize=8, label=f'Junhak CHAT(MSE:{mse_chat})')
